...
- From printed page 60 onward, primary page sections use the complete centered
  content column instead of the obsolete 704px conversion cap. Internal
  padding supplies the source-page clearance; only a component that is visibly
  narrower in the PDF may retain a narrower measured width.
...
- A Zoezi panel retains at least `32px` clearance from the preceding Mfano or
  prose block; its title bar must never touch the content above it.
...
- Vertical arithmetic in both **Mfano** and **Zoezi** uses one shared stack.
  ...
  visible `28px` gap.
- Numbered **Hatua** inside Mfano panels remain real ordered HTML lists. Their
  markers use normal weight, and consecutive steps retain a light `8px`
  vertical gap so instructions are readable without becoming over-spaced.
...
- Wide place-value arithmetic diagrams use the full inner width of their
  Mfano panel while preserving aspect ratio; this applies to the matching
  addition and subtraction figures from pages 55 onward.
...
- Keep every primary content section horizontally centred at the same print
  width as page iv. Never shrink/scale the complete section to solve height;
...
- Real top-level Mfano cards reserve `60px` above the border, reproducing the
  roughly `26px` visible gap between preceding prose and the raised label on
  source page 9. Their left edge aligns with the page's prose column.
- Prose paragraphs use source-style inter-word justification so both text
  edges are visually straight; compact component content remains left-aligned.
...
- For every extracted diagram, compare the crop with its PDF page before use:
  remove scanner noise and surrounding page pixels, preserve all labels and
  connector lines, and make only the true paper background transparent. Never
  threshold coloured artwork into a black-and-white silhouette.
- A continued review/Zoezi page keeps the same uninterrupted background and
  inner width as the page on which the exercise began. Its question number and
  first line share one grid row; wrapped text aligns with the question text,
  not with the number.
- Reverse audits are page-gated: compare the complete HTML page with the PDF,
  including its lower half and footer, before moving to the preceding page.
